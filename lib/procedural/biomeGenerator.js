...
module.exports = function createBiomeGenerator() {
    ...
    function getPercentages(neighbourList) {
        ...
        var stonePercentage = 10;
        var counts = _.countBy(neighbourList, 'biome');
        counts.autumn = counts.autumn || 0;
        counts.stone = counts.stone || 0;
        var percentages = {
            autumn: Math.floor(autumnPercentage * (0.25 * counts.autumn + 1) + (counts.stone * 2)),
            stone: Math.floor(stonePercentage * (counts.stone * 2 + 1) + (0.1 * counts.autumn))
        };
        return percentages;
    }
